...
## Update Notes`r`n- v0.6.6 | 2026-03-03 14:29 KST | Auto release scaffold generated by release_auto.cmd (version bump +0.0.01, APP_VERSION sync, git tag prep).`r`n- v0.6.6 | 2026-03-03 14:29 KST | [KR] release_auto.cmd auto release scaffold executed (+0.0.01 version, APP_VERSION sync, tag preparation).
...
- v0.6.05 | 2026-03-03 14:28 KST | 업데이트 후 다중 실행(Scenario 창 반복 생성) 문제를 단일 인스턴스 락으로 차단하고, 교체 실패 시 다운로드 EXE로 재실행하는 안전 로직으로 업데이트 스크립트를 보강.
...
- v0.6.04 | 2026-03-03 14:09 KST | 원클릭 자동 릴리즈 파이프라인(`release_auto.cmd`: 버전 +0.0.01 자동탐지, APP_VERSION 동기화, changelog 노트 추가, git 커밋/푸시/태그푸시)을 추가하고 AGENTS 규칙에 해당 절차를 강제하도록 반영.
